...
def polygons_to_mask(polygons, shape, transform):
    ...
    top_down = rasterize(shapes, out_shape=shape, transform=transform, fill=0,
...
def roof_rain_weights(building_mask: np.ndarray) -> np.ndarray:
    """Conserve rainfall on roofs by distributing it over perimeter ground cells."""
...
def prepare_inputs(dem_path: str | Path, buildings_path: str | Path,
                   vectors_path: str | Path, out_dir: str | Path,
    ...
    bdata = np.load(buildings_path, allow_pickle=True)
    vdata = np.load(vectors_path, allow_pickle=True)
    building = polygons_to_mask(bdata["buildings"], z.shape, transform)
    road = roads_to_mask(vdata["roads"] if "roads" in vdata.files else [], z.shape, transform, road_half_width)
    if "road_polygons" in vdata.files:
        road |= polygons_to_mask(vdata["road_polygons"], z.shape, transform)
    road &= ~building

    manning = np.full(z.shape, n_ground, dtype=np.float32)
    manning[road] = n_road
    ...
    np.savez_compressed(out / "metadata.npz", x=xs, y=ys, dx=np.float32(dx), N=np.int32(nr),
                        building=building.astype(np.uint8), road=road.astype(np.uint8),
                        manning=manning, rain_weight=rain_weight)
    info = {
        "N": nr,
        "dx": dx,
        "building_fraction": float(building.mean()),
        "road_fraction": float(road.mean()),
        "rain_weight_sum": float(rain_weight.sum()),
        "rain_weight_expected": int(z.size),
    }
...
def main():
    ...
    args = ap.parse_args()
    prepare_inputs(args.dem, args.buildings, args.vectors, args.out,
                   args.road_half_width, args.n_ground, args.n_road)
